...
Canonical verification docs for this repository. Cursor discovers the skill via [`.cursor/skills/verify-hugo-site/SKILL.md`](../.cursor/skills/verify-hugo-site/SKILL.md); scripts and the feature map live here.
...
Read `scripts/verify-hugo/features/README.md`, then the feature file for the behavior under test.
...
```bash
BASE="$(scripts/verify-hugo/bin/verify-hugo url)"
ART="$(scripts/verify-hugo/bin/verify-hugo artifacts)"
scripts/verify-hugo/bin/verify-hugo-screenshot --url "$BASE/" --viewport desktop --out "$ART/home/desktop.png" --aria "$ART/home/desktop.aria.txt"
scripts/verify-hugo/bin/verify-hugo-screenshot --url "$BASE/" --viewport mobile --out "$ART/home/mobile.png" --aria "$ART/home/mobile.aria.txt"
```
...
```bash
ARTIFACTS="$(scripts/verify-hugo/bin/verify-hugo artifacts)"
...
Scripts live in `scripts/verify-hugo/bin/` (must be executable). Requires project `devDependencies.playwright` and browsers from `npx playwright install chromium`.
